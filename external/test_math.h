...
TCDEF void matrix_print(matrix m);
TCDEF int matrix_equals(matrix a, matrix b);
...
TCDEF int matrix_equals(matrix a, matrix b) {
  for (int i = 0; i < 4; i++) {
    if (!vec4_equals(a.v[i], b.v[i])) return 0;
  }
  return 1;
}
...
TCDEF void matrix_scale(matrix *m, float x, float y, float z) {
  ...
  matrix u;
  matrix_clone(&u, *m);
  vec4_mul(&m->v[0], u.v[0], x);
  vec4_mul(&m->v[1], u.v[1], y);
  vec4_mul(&m->v[2], u.v[2], z);
  for (int i = 0; i < 3; i++)
